...
const SEV_CONFIG = (s) => {
    if (s >= 8) return { label: 'Critical', color: 'bg-rose-500', ring: 'ring-rose-500/20', text: 'text-rose-600' };
    if (s >= 6) return { label: 'High', color: 'bg-amber-500', ring: 'ring-amber-500/20', text: 'text-amber-600' };
    if (s >= 4) return { label: 'Medium', color: 'bg-blue-500', ring: 'ring-blue-500/20', text: 'text-blue-600' };
    return { label: 'Low', color: 'bg-slate-300', ring: 'ring-slate-400/10', text: 'text-slate-500' };
};
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
                            <div className="text-right hidden sm:block">
                                ...
                                <p className="text-[10px] text-slate-500">Auto-refresh {Math.round(REFRESH_MS / 1000)}s</p>
                            </div>
                            <div className="w-10 h-10 rounded-full bg-primary/20 border-2 border-white flex items-center justify-center">
                                <Icon name="shield_person" className="text-primary text-[20px]" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <main className="max-w-7xl mx-auto w-full px-6 pb-12 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <StatCard
                        ...
                        barWidth={totalTickets ? `w-[${Math.round((openCount / totalTickets) * 100)}%]` : 'w-0'}
                        barPct={totalTickets ? Math.round((openCount / totalTickets) * 100) : 0}
                    />
                    <StatCard
                        label="In Progress"
                        ...
                        barWidth={totalTickets ? `w-[${Math.round((inProgressCount / totalTickets) * 100)}%]` : 'w-0'}
                        barPct={totalTickets ? Math.round((inProgressCount / totalTickets) * 100) : 0}
                    />
                    <StatCard
                        label="Resolved"
                        value={resolvedCount}
                        ...
                        barWidth={totalTickets ? `w-[${Math.round((resolvedCount / totalTickets) * 100)}%]` : 'w-0'}
                        barPct={totalTickets ? Math.round((resolvedCount / totalTickets) * 100) : 0}
                    />
                </div>
                        ...
                        <div className="liquid-glass rounded-xl overflow-hidden flex flex-col h-full">
                            ...
                            <div className="p-4 border-b border-slate-200/50 flex flex-wrap gap-4 items-center justify-between bg-white/10">
                                ...
                                <div className="text-slate-400 text-sm">
                                    Showing <span className="text-slate-900 font-bold">{tickets.length}</span> report{tickets.length !== 1 ? 's' : ''}
                                </div>
                            </div>
                                        ...
                                        <tbody className="divide-y divide-slate-100">
                                            {tickets.map((t) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${STATUS_BADGE[t.status] ?? 'bg-slate-100 text-slate-600'}`}>
                                                            {(t.status ?? '').replace('_', ' ').replace(/\b\w/g, (c) => c.toUpperCase())}
                                                        </span>
                                                    </td>
                                                    <td className="px-6 py-4 text-right">
                                                        <button className="p-2 hover:bg-slate-200 rounded-lg transition-colors">
                                                            <Icon name="visibility" className="text-slate-400 group-hover:text-primary text-[20px]" />
                                                        </button>
                                                    </td>
                                                </tr>
                                            ))}
                    ...
                    <div className="lg:w-[30%] space-y-6">
                        <div className="liquid-glass rounded-xl overflow-hidden h-full flex flex-col">
                            ...
                            <div className="flex-1 relative min-h-[400px]">
                                ...
                                <div className="absolute bottom-4 left-4 right-4 liquid-glass p-4 rounded-lg flex flex-col gap-2">
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-semibold">Incident Density</span>
                                    </div>
                            ...
                            <div className="p-5 space-y-3 border-t border-slate-200/50">
                                <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400">Severity Breakdown</h4>
                                <HotspotRow
                                    color="bg-rose-500"
...
function StatCard({ label, value, icon, iconBg, accent, barColor, barPct }) {
    return (
        <div className={`liquid-glass p-5 rounded-xl group transition-all ${accent}`}>
...
function TicketModal({ ticket, onClose, onStatusChange, isUpdating }) {
    ...
    const reportedAt = ticket.createdAt
        ? new Date(ticket.createdAt).toLocaleString(undefined, { hour: 'numeric', minute: '2-digit', hour12: true, month: 'short', day: 'numeric' })
        : 'Unknown';
                ...
                <div className="flex items-center justify-between px-8 py-5 border-b border-slate-200/50 bg-white/10">
                    <div className="flex flex-col">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-slate-400 font-mono text-xs">#{ticketId}</span>
                        </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
                            ...
                            <div className="space-y-8">
                                    ...
                                    <div className="group relative overflow-hidden rounded-xl bg-slate-100 aspect-video shadow-inner border border-slate-200/50">
                                        ...
                                        <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                                            <span className="text-white text-xs font-medium flex items-center gap-1">
                                                <Icon name="zoom_in" className="text-sm" /> Click to enlarge
                                            </span>
                                        </div>
                                    </div>
                                )}
                                ...
                                {ticket.description && (
                                    <div className="space-y-3">
                                        <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Citizen Report</h3>
                                ...
                                <div className="space-y-3">
                                    <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">AI Classification</h3>
                        ...
                        <div className="lg:col-span-5 p-8 flex flex-col gap-8">
                            ...
                            {hasCoords && (
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Incident Location</h3>
                            ...
                            <div className="grid grid-cols-2 gap-y-8 gap-x-4">
                                <div>
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Time Reported</p>
                                    <div className="flex items-center gap-2">
                                        <Icon name="schedule" className="text-base text-slate-400" />
                                        <p className="text-sm font-semibold text-slate-900">{reportedAt}</p>
                                    </div>
                                </div>
                                <div>
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Category</p>
                                    <div className="flex items-center gap-2">
                                        <Icon name={CAT_ICON[ticket.aiCategory] ?? 'warning'} className="text-base text-primary" />
                                        <p className="text-sm font-semibold text-slate-900">{catLabel}</p>
                                    </div>
                                </div>
                                <div>
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Severity</p>
                                    <div className="flex items-center gap-2">
                                        <div className={`h-2 w-2 rounded-full ${sevCfg.color} ring-4 ${sevCfg.ring}`} />
                                        <span className={`text-xs font-bold ${sevCfg.text}`}>{sevCfg.label} ({ticket.severityScore}/10)</span>
                                    </div>
                                </div>
                                <div>
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Current Status</p>
                                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${STATUS_BADGE[ticket.status] ?? 'bg-slate-100 text-slate-600'}`}>
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    className={`appearance-none block w-full pl-3 pr-10 py-1.5 text-xs font-bold rounded-full border-none focus:ring-2 focus:ring-primary/20 cursor-pointer ${STATUS_SELECT_STYLE[status] ?? 'bg-slate-100 text-slate-600'}`}
                                >
                                    <option value="open">Open</option>
                                    <option value="in_progress">In Progress</option>
                                    <option value="resolved">Resolved</option>
                                </select>
                                <Icon name="expand_more" className="absolute right-2 top-1/2 -translate-y-1/2 text-sm pointer-events-none" />
                            </div>
                        </div>
                    </div>
